...
\textit{Note:} The integral formally extends to the UV cutoff $M_5$, but since we probe galactic scales $k \ll M_5$, the limit $M_{UV} \to \infty$ is a valid approximation.
...
The fundamental scale $M_5$ is not arbitrary; it is tied to the vacuum energy $\Lambda \sim L^{-4}$ via the Dark Dimension hypothesis~\cite{Vafa2022}. For $L \sim 1\,\mu\text{m}$, the Species Scale relation $M_5 \sim M_{pl}/\sqrt{N}$ yields $N \sim 10^{30}$. The leakage suppression factor is naturally derived as:
\begin{equation}
    \epsilon \equiv \frac{k_c}{k_{KK}} \sim \frac{1}{N^2} \sim 10^{-60}.
\end{equation}
This generates $k_c \sim H_0$ without fine-tuning parameters.

\section{Methodology}

\subsection{Likelihood and Data}
We analyze 118 galaxies from the SPARC database~\cite{Lelli2016}. To ensure data quality, we selected galaxies with quality flag $Q=1$ and inclination $i > 30^\circ$ to minimize geometric corrections. The Gaussian log-likelihood marginalizes over stellar Mass-to-Light ratios ($\Upsilon_\star$) with Log-Normal priors.
We compare our model against MOND using the "Standard" interpolating function $\nu(y) = (1/2 + \sqrt{1/4 + 1/y^2})^{1/2}$ as a baseline.

\textbf{Prior Sensitivity:} We repeated the nested sampling with (i) a log-uniform prior on $n \in [1.0, 2.0]$ and (ii) a broader uniform prior on $\log_{10}(a_5)$. Posterior medians shifted by $<0.1\sigma$, indicating robustness.

\textbf{CLASS Validation:} In the $k_c \to 0$ limit, our patched \texttt{CLASS} code reproduces the unmodified $\Lambda$CDM power spectrum to better than $0.01\%$.
...
The offset arises from gravitons leaking through the extra dimension, creating an apparent potential lag via WKB tunneling. This calculation is a first-order analytic estimate; full lensing simulations are left for future work.
\textbf{Sensitivity:} Varying the resonance width $\Gamma$ by one order of magnitude around $10^{-31}\,\mathrm{eV}$ yields offsets $\Delta x \in [100, 400]\,\mathrm{kpc}$. This range explicitly covers the observed separation between the X-ray gas center and the weak lensing mass peak reported by Clowe et al.~\cite{Clowe2006} and Markevitch~\cite{Markevitch2004}.
...
The modified Poisson equation leads to a scale-dependent growth factor $D(a)$. The suppression of power on small scales yields $S_8 \approx 0.78$ (Fig.~\ref{fig:s8}), alleviating the tension with weak lensing surveys~\cite{Abbott2022}.
...
\bibitem{Ooguri2007} H. Ooguri and C. Vafa, Nucl. Phys. B \textbf{766}, 21 (2007).
...
\bibitem{Deser2007} S. Deser and R. P. Woodard, Phys. Rev. Lett. \textbf{99}, 111101 (2007).
\bibitem{Maggiore2014} M. Maggiore, Phys. Rev. D \textbf{89}, 043008 (2014).
...
\bibitem{Heymans2021} C. Heymans et al. (KiDS Collaboration), Astron. Astrophys. \textbf{646}, A140 (2021).
...
\bibitem{Markevitch2004} M. Markevitch et al., Astrophys. J. \textbf{606}, 819 (2004).
