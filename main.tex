...
These challenges, alongside the Radial Acceleration Relation (RAR) \cite{McGaugh2016}, suggest departures from $\Lambda$CDM. The ``Dark Dimension'' scenario \cite{Vafa2022} suggests a micron-scale extra dimension derived from Swampland constraints. 
...
\end{equation}
The equations admit a Lifshitz-like metric $ds^2 = (z/z_0)^{2\delta} \eta_{\mu\nu} dx^\mu dx^\nu - dz^2$. As derived in Appendix A, the scaling exponent is $\delta \approx 1.23$, which determines the spectral density $\rho(\mu)$.
...
The effective coupling $\mu(k) \approx 1 + (k_c/k)^{2-n}$ confirms that gravity is modified primarily on linear scales ($k \ll k_c \approx 0.1\ h/\text{Mpc}$).
...
The statistical results for the model comparison are summarized in Table \ref{tab:evidence}. The Hybrid Leakage model shows a decisive preference over the $\Lambda$CDM+NFW limit.
...
We find $n = 1.46 \pm 0.05$. Accounting for systematic astrophysical scatter ($\sigma_{sys} \approx 0.11$ dex) and statistical error, the Z-score relative to the MOND limit ($n=1$) is:
\begin{equation}
...
The scale-dependent growth suppression (see Fig. \ref{fig:s8}) resolves the $S_8$ tension by predicting $S_8 \approx 0.78 \pm 0.02$, consistent with KiDS-1000 \cite{Heymans2021} and DES Y3 results. 

The Bullet Cluster offset is reproduced via the resonance width $\Gamma$ (Fig. \ref{fig:bullet}).
...
\section{Discussion}
The predicted slip $|\eta - 1| \approx 0.05$ serves as a falsifiable signature for Euclid. The Bullet Cluster offset is explained via the Wigner resonance mechanism, providing a fundamental basis for Weyl hysteresis.
...
The decay width $\Gamma$ is determined by the Euclidean instanton action $S \approx M_5 (\pi R) \approx 85.6$. 

This ensures cosmological stability ($\tau \gg H_0^{-1}$).

\section{Israel Junction Conditions}
Taking the trace reversed form $[K_{\mu\nu}] = -\kappa_5^2 ( S_{\mu\nu} - \frac{1}{3} S h_{\mu\nu} )$, we fix the warp factor gradient $6A'(z_b) = -\kappa_5^2 \lambda$.
